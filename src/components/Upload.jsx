...
const Upload = () => {
  ...
  }, [buttonsDisabled, selectedImageIndex]);
...
const handleAccept = async () => {
  if (selectedImageIndex !== null && imageSets.length > 0) {
    ...
    for (const image of currentSet) {
      const fileHandle = await batchDirectory.getFileHandle(image.file.name, {
        create: true,
      });
    ...
    // Set the selected image index to the first image of the next set or the first image if there's no next set
    setSelectedImageIndex(
      currentSetIndex === imageSets.length - 1 ? 0 : (currentSetIndex + 1) * 8
    );

    // Reset folderSelected state variable if needed
    // setFolderSelected(false); // No need to reset folderSelected here
  } else {
...
const handleReject = async () => {
  if (selectedImageIndex !== null && imageSets.length > 0) {
    ...
    const rejectFolder = await rejectFolderHandle.getDirectoryHandle(`Reject`, {
      create: true,
    });
    ...
    for (const image of currentSet) {
      const fileHandle = await rejectFolder.getFileHandle(image.file.name, {
        create: true,
      });
      const writable = await fileHandle.createWritable();
      await fetch(image.url)
